...
date    = str(sys.argv[1])
mydir   = str(sys.argv[2])
outdir = mydir

# Define file to use
datfile = mydir+f'REF_y{date[0:4]}m{date[4:6]}d{date[6:8]}.nc'
print(datfile)

ds = xr.open_dataset(datfile)
...
df['deptht'] = ('z', dep)
df.to_netcdf(mydir+'REF_y'+date[0:4]+'m'+date[4:6]+'d'+date[6:8]+'_refl.nc')
